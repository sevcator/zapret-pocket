...
# discordfakesplit_z2.sh
# source-profile: discordfakesplit (zapret2 / nfqws2 translation)
# Translated from discordfakesplit.sh per _zapret2_mapping.md (CORRECTED FULL SPEC).
ZAPRET=2

config="--blob=fake_tls_3:@$MODPATH/zapret/tls_clienthello_3.bin --blob=fake_tls_google:@$MODPATH/zapret/tls_clienthello_www_google_com.bin --blob=fake_quic_google:@$MODPATH/zapret/quic_initial_www_google_com.bin --filter-tcp=443 --ipset=$MODPATH/list/ipset-all.txt --hostlist-exclude=$MODPATH/list/list-exclude.txt --hostlist-exclude=$MODPATH/list/list-exclude-user.txt --ipset-exclude=$MODPATH/list/ipset-exclude.txt --ipset-exclude=$MODPATH/list/ipset-exclude-user.txt --payload=tls_client_hello --lua-desync=fake:blob=fake_tls_3:ip_ttl=2 --lua-desync=multisplit:pos=sniext:seqovl=1 --new"
config="$config --filter-tcp=443 --hostlist=$MODPATH/list/list-google.txt --payload=tls_client_hello --lua-desync=fake:blob=fake_tls_google:repeats=6:tcp_seq=-10000 --lua-desync=multisplit:pos=2 --new"
...
if [ "$(cat "$MODPATH/config/bypass-calls" 2>/dev/null || echo 0)" = "1" ]; then
    ...
    config="$config --filter-udp=19294-19344,50000-50100 --filter-l7=discord,stun --payload=stun,discord_ip_discovery --out-range=-d3 --lua-desync=fake:blob=0x00000000000000000000000000000000:payload=all:repeats=6 --new"
fi

...
# YTDisBystro_31_1_z2.sh
# source-profile: YTDisBystro_31_1 (translated to zapret2 / nfqws2)
# Generated from zapret-pocket-main strategies, --dpi-desync* translated to --lua-desync.

ZAPRET=2

config="--blob=syn_data:@$MODPATH/zapret/tls_clienthello_7.bin --blob=fake_tls_14:@$MODPATH/zapret/tls_clienthello_14.bin --blob=fake_quic_www_google_com:@$MODPATH/zapret/quic_initial_www_google_com.bin --blob=fake_quic_6:@$MODPATH/zapret/quic_6.bin --blob=fake_tls_www_google_com:@$MODPATH/zapret/tls_clienthello_www_google_com.bin --filter-tcp=443 --ipset=$MODPATH/list/ipset-all.txt --hostlist-exclude=$MODPATH/list/list-exclude.txt --hostlist-exclude=$MODPATH/list/list-exclude-user.txt --ipset-exclude=$MODPATH/list/ipset-exclude.txt --ipset-exclude=$MODPATH/list/ipset-exclude-user.txt --lua-desync=syndata:blob=syn_data --new"
...
config="$config --filter-udp=443 --hostlist=$MODPATH/list/list-general.txt --hostlist=$MODPATH/list/list-general-user.txt --hostlist-exclude=$MODPATH/list/list-exclude.txt --hostlist-exclude=$MODPATH/list/list-exclude-user.txt --ipset-exclude=$MODPATH/list/ipset-exclude.txt --ipset-exclude=$MODPATH/list/ipset-exclude-user.txt --payload=quic_initial --out-range=-n3 --lua-desync=fake:blob=fake_quic_6:repeats=2:ip_autottl=-1,3-20 --lua-desync=udplen:increment=8:pattern=0x0F0F0E0F --new"

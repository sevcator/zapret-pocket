...
config="$config --filter-udp=443 --ipset=$MODPATH/list/ipset-all.txt --hostlist-exclude=$MODPATH/list/list-exclude.txt --hostlist-exclude=$MODPATH/list/list-exclude-user.txt --ipset-exclude=$MODPATH/ipset/ipset-exclude.txt --ipset-exclude=$MODPATH/ipset/ipset-exclude-user.txt  --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-any-protocol=1 --dpi-desync-fake-quic=$MODPATH/zapret/quic_initial_www_google_com.bin --new"

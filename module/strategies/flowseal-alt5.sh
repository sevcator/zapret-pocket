...
config="$config --filter-tcp=443 --hostlist=$MODPATH/list.txt --dpi-desync=fake,split2 --dpi-desync-repeats=6 --dpi-desync-fooling=md5sig --dpi-desync-fake-tls=$MODPATH/tls_clienthello_www_google_com.bin --new"
...
config="$config --filter-tcp=443 --ipset=$MODPATH/ipset-host.txt --dpi-desync=fake,split2 --dpi-desync-repeats=6 --dpi-desync-fooling=md5sig --dpi-desync-fake-tls=$MODPATH/tls_clienthello_www_google_com.bin --new"
config="$config --filter-tcp=443 --hostlist-auto=$MODPATH/list-auto.txt --dpi-desync=fake,split2 --dpi-desync-repeats=6 --dpi-desync-fooling=md5sig --dpi-desync-fake-tls=$MODPATH/tls_clienthello_www_google_com.bin --new"
...
config="--filter-tcp=80 --dpi-desync=fake,split2 --dpi-desync-fooling=md5sig $hostlist --new"
...
config="$config --filter-udp=50000-50099 --ipset=$MODPATH/ipset-discord.txt --dpi-desync=fake --dpi-desync-any-protocol --dpi-desync-cutoff=n1 --dpi-desync-fake-quic=$MODPATH/quic-google.bin --new"
